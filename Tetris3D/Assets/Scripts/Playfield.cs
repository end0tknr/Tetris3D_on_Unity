...
public class Playfield : MonoBehaviour
{
    ...
    //Header構文により unityのinspector欄にヘッダとして表示
    [Header("Blocks")]
    ...
    public void SpawnNewBlock()
    {
        Vector3 spawnPoint =
            new Vector3(
                ...
                (int)transform.position.y + (float)gridSizeY,
                (int)(transform.position.z + (float)gridSizeZ / 2));
        //int randomIndex = Random.Range(0, blockList.Length);
        //SPAWN THE BLOCK
        GameObject newBlock =
            Instantiate(blockList[randomIndex],spawnPoint,Quaternion.identity)
            as GameObject;
        //GHOST
        GameObject newGhost =
            Instantiate(ghostList[randomIndex], spawnPoint, Quaternion.identity)
            as GameObject;
    ...
    void MoveAllLayerDown(int y)
    {
        ...
        {
            MoveOneLayerDownAt(i);
        }
    }

    void MoveOneLayerDownAt(int y)
    {
        for (int x = 0; x < gridSizeX; x++)
        {
            for (int z = 0; z < gridSizeZ; z++)
    ...
    void OnDrawGizmos()
    {
        if (bottomPlane != null)
        {
            //RESIZE
            Vector3 scaler = 
                new Vector3((float)gridSizeX / 10, 1, (float)gridSizeZ / 10);
            bottomPlane.transform.localScale = scaler;

            //REPOSITION
            bottomPlane.transform.position =
                new Vector3(
        ...
        if (N != null)
        {
            //RESIZE
            Vector3 scaler =
                new Vector3((float)gridSizeX / 10, 1, (float)gridSizeY / 10);
            ...
            //RETILE MATERIAL
            N.GetComponent<MeshRenderer>().sharedMaterial.mainTextureScale =
        ...
        if (E != null)
        {
            //RESIZE
            Vector3 scaler =
                new Vector3((float)gridSizeZ / 10, 1, (float)gridSizeY / 10);
            ...
            E.transform.position =
                new Vector3(
                    ...
                    transform.position.z + (float)gridSizeZ / 2);
            //RETILE MATERIAL
            E.GetComponent<MeshRenderer>().sharedMaterial.mainTextureScale =
